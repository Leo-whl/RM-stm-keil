...
void chassis_task(void const *pvParameters)
{
    ...
    //判断底盘电机是否都在线
    while ( toe_is_error(CHASSIS_MOTOR3_TOE) || toe_is_error(CHASSIS_MOTOR4_TOE)  || toe_is_error(DBUS_TOE))    //toe_is_error(CHASSIS_MOTOR1_TOE) || toe_is_error(CHASSIS_MOTOR2_TOE)||
    {
    ...
    while (1)
    {
        ...
        if (!( toe_is_error(CHASSIS_MOTOR3_TOE) && toe_is_error(CHASSIS_MOTOR4_TOE)))//toe_is_error(CHASSIS_MOTOR1_TOE) && toe_is_error(CHASSIS_MOTOR2_TOE) &&
        {
...
uint16_t z_buffer=0,z_buffer_=0;
float vz_ponson=0.0f;
extern ext_game_state_t game_state;
uint8_t spin_flag=0;
#define LOW_SPEED 7
#define HIGH_SPEED 13
/**
  * @brief          set chassis control set-point, three movement control value is set by "chassis_behaviour_control_set".
  * @param[out]     chassis_move_update: "chassis_move" valiable point
  * @retval         none
...
static void chassis_set_contorl(chassis_move_t *chassis_move_control)
{
    ...
    {
			z_buffer=0,z_buffer_=0;vz_ponson=0.0f;
...
//        chassis_move_control->wz_set = PID_calc(&chassis_move_control->chassis_angle_pid, 0.0f, delat_angle);
        //speed limit
        //速度限幅
			if((game_state.game_progress == 4) || (switch_is_mid(chassis_move.chassis_RC->rc.s[CHASSIS_SPIN_CHANNEL])))
			{
				if(spin_flag)
				{
				  if(vz_ponson>LOW_SPEED)
					{
						if(z_buffer_++ > 5)
						{
							z_buffer_=0;
			...
			else 
			{
				...
				chassis_move_control->wz_set = vz_ponson;
