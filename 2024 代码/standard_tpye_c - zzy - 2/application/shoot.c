...
/**
  * @brief          射击控制,左键按下或者拨杆拨下拨弹轮会进行旋转
  * @param[in]      void
  * @retval         void
  */
static void shoot_biubiubiu(void);
...
//    if (switch_is_mid(shoot_control.shoot_rc->rc.s[SHOOT_RC_MODE_CHANNEL]) && (shoot_control.shoot_rc->key.v & SHOOT_ON_KEYBOARD) && shoot_control.shoot_mode == SHOOT_STOP)
		if(switch_is_mid(shoot_control.shoot_rc->rc.s[SHOOT_RC_MODE_CHANNEL]) && (robot_state.mains_power_shooter_output == 1) && shoot_control.shoot_mode == SHOOT_STOP)
    {
			
			if(shoot_time_buffer < 200) shoot_time_buffer++;
			else 
			{
				shoot_time_buffer=0;shoot_time++;
			}
			if(shoot_time>7)
			{
			...
			else shoot_control.shoot_mode = SHOOT_STOP;
    }
    //处于中档， 可以使用键盘关闭摩擦轮
//    else if (switch_is_mid(shoot_control.shoot_rc->rc.s[SHOOT_RC_MODE_CHANNEL]) && (shoot_control.shoot_rc->key.v & SHOOT_OFF_KEYBOARD) && shoot_control.shoot_mode != SHOOT_STOP)
		else if (switch_is_mid(shoot_control.shoot_rc->rc.s[SHOOT_RC_MODE_CHANNEL]) && (robot_state.mains_power_shooter_output == 0) && shoot_control.shoot_mode != SHOOT_STOP)
    {
        shoot_control.shoot_mode = SHOOT_STOP;
    }
...
//    if(!toe_is_error(REFEREE_TOE) && (shoot_control.heat + SHOOT_HEAT_REMAIN_VALUE > shoot_control.heat_limit))
		if(!toe_is_error(REFEREE_TOE) && (shoot_control.heat > shoot_control.heat_limit * 0.66f))
    {
...
static void shoot_biubiubiu(void)
{
	if(switch_is_down(shoot_control.shoot_rc->rc.s[SHOOT_RC_MODE_CHANNEL]) || shoot_control.press_l)
	{
		shoot_control.trigger_speed_set = TRIGGER_SPEED;
	}
	else if (shoot_control.press_r)
	{
		shoot_control.trigger_speed_set = 15.0f;
	}
	else 
	{
