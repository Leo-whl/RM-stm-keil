...
//#define SERVO1_ADD_PWM_KEY  KEY_PRESSED_OFFSET_R
...
const static uint16_t servo_key = SERVO1_ADD_PWM_KEY;
...
void servo_task(void const * argument)
{
    ...
    while(1)
    {
		...
		{
				//while((servo_rc->key.v & servo_key[4])|servo_rc->rc.ch[4])
//			      while((servo_rc->key.v & servo_key)|servo_rc->rc.ch[4])
			while((servo_rc->key.v & servo_key))
				{
						 servo_pwm_set(SERVO_MAX_PWM, i);
				
				}
				servo_pwm_set(SERVO_MIN_PWM, i);
		}
/*        for(uint8_t i = 0; i < 4; i++)
        {
