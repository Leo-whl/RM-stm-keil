...
//    static fp32 const add_time = PI * 0.5f * configTICK_RATE_HZ / CHASSIS_CONTROL_TIME_MS;
    ...
    //计算遥控器的原始输入信号
		...
		//小陀螺：要做的就是把旋转角度算出来
		if ((chassis_move_rc_to_vector->chassis_RC->key.v & REVOLVE_START_KEY)|(chassis_move_rc_to_vector->chassis_RC->rc.ch[4]))
		{
		//第一种方法	
		revolve_add=0.11f;//0.08
...
//			
		...
		else if((chassis_move_rc_to_vector->chassis_RC->key.v & REVOLVE_STOP_KEY)|!(chassis_move_rc_to_vector->chassis_RC->rc.ch[4]))
		{
		revolve_add=0.0f;
		revolve_flag=0;
		}
