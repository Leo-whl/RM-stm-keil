...
static uint8_t usb1_buf[256];
static void usb1_printf(const char *fmt,...);
...
//    usart1_tx_dma_enable(usart_tx_buf, 20);
		usb1_printf("%f, %f, %f, %f  \r\n",gimbal_control.gimbal_yaw_motor.absolute_angle,gimbal_control.gimbal_yaw_motor.absolute_angle_set,gimbal_control.gimbal_yaw_motor.motor_gyro,gimbal_control.gimbal_yaw_motor.motor_gyro_set);
}
...
static void usb1_printf(const char *fmt,...)
{
    ...
    len = vsprintf((char *)usb1_buf, fmt, ap);

    va_end(ap);

		usart1_tx_dma_enable(usb1_buf, len);
}
