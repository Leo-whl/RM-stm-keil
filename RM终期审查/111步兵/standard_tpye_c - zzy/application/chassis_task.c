...
static void chassis_init(chassis_move_t *chassis_move_init)
{
    ...
    uint8_t i;
		...
		//小陀螺旋转PID值
		//const static fp32 revolve_pid[3]={ CHASSIS_REVOLCE_PID_KP,CHASSIS_REVOLCE_PID_KI,CHASSIS_REVOLCE_PID_KD};   //更改
...
extern uint8_t IsGimbalMotionless(void);			//更改
uint8_t revolve_flag=0;							//更改
/**
  * @brief          set chassis control set-point, three movement control value is set by "chassis_behaviour_control_set".
  * @param[out]     chassis_move_update: "chassis_move" valiable point
  * @retval         none
...
static void chassis_set_contorl(chassis_move_t *chassis_move_control)
{
    ...
    if (chassis_move_control->chassis_mode == CHASSIS_VECTOR_FOLLOW_GIMBAL_YAW)
    {
        ...
        chassis_move_control->vy_set = fp32_constrain(chassis_move_control->vy_set, chassis_move_control->vy_min_speed, chassis_move_control->vy_max_speed);
			...
			{ 
				//revolve_angle在数据更新里 ove_control->revolve_angle);
//        cos_yaw = arm_cos_f32(-chassis_move_control->revolve_angle);
        ...
        chassis_move_control->chassis_relative_angle_set = rad_format(angle_set)+chassis_move_control->chassis_yaw_motor->relative_angle;
				        ...
				        //计算旋转PID角速度
        chassis_move_control->wz_set = 17.5f*(-PID_calc(&chassis_move_control->chassis_angle_pid, chassis_move_control->chassis_yaw_motor->relative_angle, chassis_move_control->chassis_relative_angle_set));
      //第二种方法，直接给一个确定的遥控值,直接给的是速度
//				chassis_move_control->wz_set = CHASSIS_WZ_RC_SEN * angle_set;
			}
			
			
				if(IsGimbalMotionless())                    //更改
				{
